...
func InitTable(db Service) {
	...
	_, err := dbInstance.Exec(config.TablesQueries)
	if err != nil {
...
func (s *service) Health() map[string]string {
	...
	if dbStats.OpenConnections > 40 { // Assuming 50 is the max for this example
		...
		log.Warn().Str("message", stats["message"]).Msg("Database load warning")
	}

	if dbStats.WaitCount > 1000 {
		stats["message"] = "The database has a high number of wait events, indicating potential bottlenecks."
		log.Warn().Str("message", stats["message"]).Msg("Database wait event warning")
	}

	if dbStats.MaxIdleClosed > int64(dbStats.OpenConnections)/2 {
		stats["message"] = "Many idle connections are being closed, consider revising the connection pool settings."
		log.Warn().Str("message", stats["message"]).Msg("Idle connections warning")
	}

	if dbStats.MaxLifetimeClosed > int64(dbStats.OpenConnections)/2 {
		stats["message"] = "Many connections are being closed due to max lifetime, consider increasing max lifetime or revising the connection usage pattern."
		log.Warn().Str("message", stats["message"]).Msg("Max lifetime closed warning")
	}
...
func (s *service) Close() error {
	log.Info().Msgf("Disconnected from database: %s", config.DatabaseName)
	return s.db.Close()
}

// ------- GET -------

func GetUser(email string) (models.CompleteUser, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return models.CompleteUser{}, fmt.Errorf("no user found with email: %s", email)
		}
		return models.CompleteUser{}, fmt.Errorf("error querying user: %w", err)
	}

	return user, nil
}

func GetSurvey(email string) (models.Survey, error) {
	var survey models.Survey

	if !CheckIfUserExists(email) {
		return models.Survey{}, fmt.Errorf("user with email %s already exists", email)
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return models.Survey{}, fmt.Errorf("no survey found for email: %s", email)
		}
		return models.Survey{}, fmt.Errorf("error querying survey: %w", err)
	}
...
func GetPhoto(email string) ([]byte, error) {
	...
	log.Debug().Int("ImageID", imageID).Msg("ImageID received")
	...
	log.Debug().Uint32("loOID", loOID).Msg("loOID received")
	...
	defer func() {
		...
		if closeErr != nil {
			log.Error().Err(closeErr).Msg("failed to close large object")
		}
	}()
...
func AddUser(user models.User) error {
	...
	log.Debug().Msgf("User added successfully: %s (Rows affected: %d)", user.Email, rowsAffected)

	return nil
}

func AddUserInfo(user models.UserInfo, email string) error {
	...
	log.Debug().Msgf("User info updated successfully for email %s (Rows affected: %d)", email, rowsAffected)

	return nil
}

func AddSurvey(survey models.Survey, email string) error {
	log.Debug().Str("Email", email).Msg("Email received")
	log.Debug().Str("Response", survey.Response).Msg("Response received")
	...
	log.Debug().Str("SurveyID", surveyID).Msg("Survey ID generated")

	queryUpdate := "UPDATE users SET id_survey = $1 WHERE email = $2"
	result, err := tx.Exec(queryUpdate, surveyID, email)
	if err != nil {
		log.Error().Err(err).Str("SurveyID", surveyID).Str("Email", email).Msg("Failed to update user with survey ID")
	...
	log.Debug().Str("Email", email).Str("SurveyID", surveyID).Msg("Survey successfully linked to user")
